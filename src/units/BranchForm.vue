<template>
  <div class="add-form" @click.stop="">
    ...
    <div class="add-form-cont">
      <Drop :conf="typeConf" v-if="!newBranchConf.val"/>
      <Drop :conf="oriBranchConf" v-if="!newBranchConf.val"/>
      <Drop :conf="newBranchConf"/>
    </div>
...
<script>
  ...
  export default {
    ...
    data() {
      return {
        typeConf: {val: "local", name: "分支类型", list: ["local", "remote"]},
        oriBranchConf: {val: "", name: "已有分支", list: []},
        newBranchConf: {val: "", name: "新分支"},
      }
    },
    methods: {
      onConfirm() {
        this.conf.onConfirm && this.conf.onConfirm(this.newBranchConf.val || this.oriBranchConf.val);
      },
      onCancle() {
        this.conf.hide();
      },
