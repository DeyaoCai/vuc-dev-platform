...
export default {
  ...
  methods: {
    getList(){
      ...
      fetch(`http://localhost:9995/docker/images?${Object.keys(data).map(item => (`${item}=${data[item]}`)).join("&")}`,{method: "GET"}).then(res =>{
        res.json().then(res=>{
    ...
    getListData(){
      ...
      console.log(this.sort.startDate, new Date(this.sort.startDate).getTime())
      const ret = {
        name: this.sort.name,
        startDate: startDate,
        endDate: endDate,
